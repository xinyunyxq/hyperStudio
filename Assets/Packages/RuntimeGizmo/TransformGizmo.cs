...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						for(int i = 0; i < targetRootsOrdered.Count; i++)
						{
							...
							//--yexqs
							var distance = (target.transform.position - Camera.main.transform.position).magnitude;
							//--yexqe
							...
							//if (movement.z == 0f)
							{
								...
								target.transform.position = (target.position - Camera.main.transform.position).normalized * distance + Camera.main.transform.position;
								// keep the object facing the camera
								if (Config.instance.AutoLookAtCamera)
								{
									target.transform.LookAt(2 * target.transform.position - Camera.main.transform.position);
								}
							}
							//--yexqe
						}
		...
		public void deleteTargetFromScene()
        {
			UndoRedoManager.Clear();
			for (int i = 0; i < targetRootsOrdered.Count; i++)
			{
				Destroy(targetRootsOrdered[i].gameObject);
			}
			ClearTargets(false);
		}
