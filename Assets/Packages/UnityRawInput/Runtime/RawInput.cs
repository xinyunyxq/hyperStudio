...
namespace UnityRawInput
{
    public static class RawInput
    {
        ...
        /// Event invoked when user releases a key.
        /// </summary>
        public static event Action<MouseArgs> OnMouseMove;
        ...
        private static int HandleMouseProc (int code, IntPtr wParam, IntPtr lParam)
        {
            ...
            if (state == RawMouseState.LeftButtonDown)
            {
                HandleKeyDown(RawKey.LeftButton);
            }

            else if (state == RawMouseState.LeftButtonUp) {
                HandleKeyUp(RawKey.LeftButton);
            }
            ...
            else if (state == RawMouseState.MouseMove) HandleMouseMove(args);

            return (InterceptMessages||LockMouse) ? 1 : Win32API.CallNextHookEx(IntPtr.Zero, 0, wParam, lParam);
        ...
        private static void HandleMouseMove(MouseArgs args)
        {
